              ...
              <div className="item"> 
                         {
                          this.props.arr.map((item,index) => { //这个地方通过this.props.arr接收到父组件传过来的arr，然后在{}里面进行js的循环
                              return ( 
                                             <div key={index}> 
                                             <Row>
                                              <Col offset={4} span={20} style={{paddingBottom:10}}>红包{index+1}{index!="0"? (<span  onClick={()=>t.props.parentdelOneItem(index)}  style={{color:'#09c',cursor:"pointer"}}>删除</span>):null}</Col>
                                              </Row>
